...
# class to approximate p( a | s)
class PolicyModel:
	def __init_(self, D, K, hidden_layer_sizes):     #D=input_size, K=output_size, list of all hidden layers sizes
		...
		self.actions = tf.placeholders(tf.int32, shape=(None,), name='actions')
		self.advantages = tf.placehodlder(tf.float32, shape=(None,), name='advantages')
		...
		selected_probs = tf.log(tf.reduce_sum(p_a_given_s * tf.one_hot(self.actins, K), reduction_indices=[1]))
...
# class to approximate state value function V(s)
class ValueModel:
