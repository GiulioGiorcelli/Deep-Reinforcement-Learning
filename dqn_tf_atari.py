...
IM_SIZE = 80
K = 4 # (env.actiopn_space.n)



# function to get rid of useless stuff in action space
def downsample_image(A):
	B = A[31:195]      # these pixel coordinated get rid of things like frames, scoreboard and useless graphics
	B = B.mean(axis=2) # greyscale conversion
	
	# now we reshape image into a square. this changes aspect ratio but it does not distort the image
...
class DQN:
	def __init__(self, K, conv_layer_sizes, hidden_layer_sizes, gamma, scope):
		...
		with tf.variable_scope(scope):
			...
			# tensorflow conv : (num_samples, height, width, color) - need to transpose layer
...
def learn(model, target_model, experience_replay_buffer, gamma, batch_size):
	# sample experiences
	samples = random.sample(experience_replay_buffer)
	states, actions, rewards, next_states, dones = map(np.array, zip(*samples))
	
	# calculate targets
	next_Qs = target_model.predict(next_states)
	next_Q = np.amax(next_Qs, axis=1)
	taregets = rewards + np.invert(dones).astype(np.float32) * gamma * next_Q
...
def play_one(env, total_t, experience_replay_buffer, model, target_model, gamma,
	...
	while not done:
		...
		# save latest experience
		experience_replay_buffer.append((state, action, reward, next_state, done))
		
		# train model keeping track of time
		t0_2 = datetime.now()
